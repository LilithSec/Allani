-- Allani schema version 10
--
-- Full schema at version 10: syslog + http_access + http_error, the raw GIN
...
--
-- The log tables carry flat analyze (v7) and vacuum (v8) thresholds rather
-- than the server's proportional defaults, so both keep pace as a store grows
-- instead of falling further behind. See upgrade/6-7 and upgrade/7-8 for why
-- that matters.
--
-- The (column, r_isodate) indexes (v9) sit alongside the (column, id) ones
-- rather than replacing them: the first pair answer "does this value appear in
-- the window", the second "the newest N rows for this value". See upgrade/8-9.
--
-- The _ts_dims covering indexes (v10) answer a third question, "what values are
-- in the window and how many of each", from the index rather than the table.
-- They need a populated visibility map to pay -- see upgrade/9-10.
...
CREATE INDEX syslog_ts_dims ON syslog (r_isodate)
    INCLUDE (program, facility, host, host_from, priority, sourceip);

CREATE INDEX syslog_host_r_isodate    ON syslog (host, r_isodate);
CREATE INDEX syslog_program_r_isodate ON syslog (program, r_isodate);
...
CREATE INDEX http_access_ts_dims ON http_access (r_isodate)
    INCLUDE (vhost, client_ip, host, method, status);

CREATE INDEX http_access_vhost_r_isodate     ON http_access (vhost, r_isodate);
CREATE INDEX http_access_client_ip_r_isodate ON http_access (client_ip, r_isodate);
...
CREATE INDEX http_error_ts_dims ON http_error (r_isodate)
    INCLUDE (server, client_ip, code, host, loglevel);

CREATE INDEX http_error_client_ip_r_isodate ON http_error (client_ip, r_isodate);
CREATE INDEX http_error_server_r_isodate    ON http_error (server, r_isodate);
