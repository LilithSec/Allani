-- Allani schema version 7
--
-- Full schema at version 7: syslog + http_access + http_error, the raw GIN
...
--
-- The log tables carry a flat analyze threshold (v7) rather than the server's
-- proportional default, so the planner's statistics keep pace as a store grows
-- instead of falling further behind. See upgrade/6-7 for why that matters.
